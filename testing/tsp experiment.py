...
MIN_2: int = 1
DEBUG: bool = False
DEBUGGRAPH: bool = True
CUSTOMPATH = [8, 0, 9, 10, 4, 6, 3, 1, 7, 2, 5]
...
def printWithEdgeWeightNUM(ll: LinkedList):
  walk: Node = ll.start
  nums = np.loadtxt(FILENAME, dtype="i", delimiter=" ")
  tsp = [[j for j in vert] for i, vert in enumerate(nums)]
  s = str(ll.end.data) + "-" + str(tsp[ll.start.data][ll.end.data]) + "-"

  while walk.end != None:
    s += str(walk.data) + "-" + str(tsp[walk.data][walk.end.data]) + "-"
    walk = walk.end
  s += str(walk.data)
  print(s)
...
printLL(tsp[0])
printWithEdgeWeight(ll)
print()
printWithEdgeWeightNUM(tsp[0])


## ===================================
#HEAD Handles the sum of a custom path
## ===================================
if CUSTOMPATH:
  startNode = Node(CUSTOMPATH[0])
  walk = startNode
  walkNext = None

  for i, key in enumerate(CUSTOMPATH):
    if i >= len(CUSTOMPATH)-1:
      break
    walkNext = Node(CUSTOMPATH[i+1])
    walk.end = walkNext
    walk = walkNext
    
  ll = LinkedList(startNode, walk)

  print()
  print("Minimum:" + str(getTot(ll)))
  printLL(ll)
  printWithEdgeWeight(ll)
  print()
  printWithEdgeWeightNUM(ll)
